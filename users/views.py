...
mechanism to the new user via the EmailThread mechanism.
...
to the user via the EmailThread mechanism.
...
This class is called via a url mapping which also passes via the url the encoded uid base 64 token. The user is retrived from the model after the
token is decoded and checked (via primary key). If the current user is not activated - as it should be for a new user, the is_active boolean 
is set in the users model which then via the Django authentication mechanism, enables the user to log into the application while being
